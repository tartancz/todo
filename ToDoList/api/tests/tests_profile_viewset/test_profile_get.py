...
def test_get_list_profile_not_authenticated(api_client, user, rf):
    ...
    assert data["results"][0]["profile_api"] == reverse(
        "profile-detail", args=[user.profile.id], request=rf.request()
    )
    assert data["results"][0]["profile_web"] == reverse(
        "profile", args=[user.profile.id], request=rf.request()
    )
...
def test_get_detail_profile_not_authenticated(api_client, user, rf):
    ...
    assert data["profile_api"] == reverse(
        "profile-detail", args=[user.profile.id], request=rf.request()
    )
    assert data["profile_web"] == reverse(
        "profile", args=[user.profile.id], request=rf.request()
    )
    assert len(data["todo"]["results"]) == data["todo"]["count"]
    assert (
        len(data["todo"]["results"])
        == ToDo.objects.filter(Q(public=True) & Q(created_by=user)).count()
    )
    assert response.status_code == 200
